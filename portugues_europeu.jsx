...
const APP_META = {
  version: "2.7.9", // ALWAYS update the <!-- version: X.Y.Z --> comment in <head> to match
...
const CHIP_CARVING_VOCAB = [
  ...
  { id: "cc-verbs", label: "Carving Verbs", items: [
    ...
    { en: "to resume",   pt: "retomar",     referent: "continuing an action",        pos: "verb", obj: "o corte" },
...
const ChipCarvingTab = React.memo(function ChipCarvingTab({ fontSize, speakListPT }) {
  ...
  const ttsText = (item) => {
    // verbs: speak infinitive only; phrases: full PT; nouns: PT term
    return item.pt;
  };
  ...
  return (
    <div style={listPanelStyle}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 12, paddingBottom: 8, borderBottom: "1px solid var(--color-border-tertiary)" }}>
        <button
          style={activeSection === "all" ? pillActive : pillBase}
          onClick={() => { setActiveSection("all"); setOpenSection(null); }}>
          All
        </button>
        {CHIP_CARVING_VOCAB.map(sec => (
          <button
            ...
            onClick={() => { setActiveSection(sec.id); setOpenSection(null); }}>
            {sec.label}
          </button>
        ))}
      </div>
