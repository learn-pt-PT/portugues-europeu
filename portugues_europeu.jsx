...
const MINIMAL_PAIRS = [
  ...
  { a: { word: "mãe", meaning: "mother" }, b: { word: "mês", meaning: "month" } },
  ...
  { a: { word: "fato", meaning: "suit" }, b: { word: "fado", meaning: "fado music" } },
  { a: { word: "pico", meaning: "peak" }, b: { word: "bico", meaning: "beak" } },
  { a: { word: "copo", meaning: "glass" }, b: { word: "corpo", meaning: "body" } },
  ...
  { a: { word: "vale", meaning: "valley" }, b: { word: "bule", meaning: "kettle" } },
    ...
    <div style={{ maxWidth: "80%", alignSelf: isAssistant ? "flex-start" : "flex-end", display: "flex", flexDirection: "column", gap: 3 }}>
      ...
      {m._retryable && onRetry && (
        <button onClick={() => onRetry(m._retryText)}
          style={{ alignSelf: "flex-start", background: "none", border: "1px solid #dc2626", borderRadius: "var(--border-radius-md)", cursor: "pointer", fontSize: 12, color: "var(--color-text-danger)", padding: "2px 8px", marginTop: 2 }}
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", padding: "12px 16px", gap: 10 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        {cardBtn(pairsQuizMode ? "📋 Browse" : "🎯 Quiz", toggleMode, { background: pairsQuizMode ? "#eff6ff" : "var(--color-background-green)", color: pairsQuizMode ? "var(--color-accent-blue)" : "var(--color-accent-green)", borderColor: pairsQuizMode ? "var(--color-border-info)" : "var(--color-border-green)" })}
          ...
          <div style={{ display: "flex", gap: 20, justifyContent: "center", flexWrap: "wrap" }}>
            {["a", "b"].map(side => (
              <div key={side} style={{ textAlign: "center", background: "var(--color-background-secondary)", border: "1px solid var(--color-border-tertiary)", borderRadius: 12, padding: "18px 28px", minWidth: 140 }}>
                ...
                <button onClick={() => speakListPT(pair[side].word)} style={{ fontSize: 13, padding: "4px 14px", borderRadius: 6, border: "1px solid #bfdbfe", background: "#eff6ff", color: "var(--color-accent-blue)", cursor: "pointer" }}>▶ Ouvir</button>
              </div>
            ))}
          </div>
        ) : (
          ...
          <div style={{ textAlign: "center", width: "100%", maxWidth: 360 }}>
            {!quizTarget ? (
              <div>
                ...
                {cardBtn("▶ Play", startQuiz, { fontSize: fontSize, padding: "10px 32px", background: "#eff6ff", color: "var(--color-accent-blue)", borderColor: "var(--color-border-info)" })}
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                <button onClick={playQuizWord} style={{ fontSize: 16, padding: "10px 32px", borderRadius: 6, border: "1px solid #bfdbfe", background: "#eff6ff", color: "var(--color-accent-blue)", cursor: "pointer" }}>▶ Play again</button>
                {quizResult === null ? (
...
const NumbersTab = React.memo(function NumbersTab({ fontSize, speakListPT, speechSupported }) {
  ...
  const [category, setCategory]       = S("all");   // "all" | "cardinals" | "other"

  // Active pool derived from category selection.
  const activePool = M(() => {
    if (category === "cardinals") return NUMBERS_QUIZ_POOLS.cardinals;
    if (category === "other")     return NUMBERS_QUIZ_POOLS.other;
    return [...NUMBERS_QUIZ_POOLS.cardinals, ...NUMBERS_QUIZ_POOLS.other];
  }, [category]);

  // ── Quiz state ──
  const [order, setOrder]             = S(() => makeOrder(NUMBERS_QUIZ_POOLS.cardinals.length + NUMBERS_QUIZ_POOLS.other.length));
  ...
  function startMic(lang) {
    ...
    r.onend   = () => { setMicActive(false); micRef.current = null; if (finalText.trim()) evaluateMic(finalText.trim()); };
    micRef.current = r;
    r.start();
    setMicActive(true);
    setTranscript("");
  ...
  function evaluateMic(raw) {
    const heard = normaliseAnswer(raw);
    let correct = false;
    if (direction === "pt-en") {
      correct = heard === normaliseAnswer(item.en);
    } else {
      // EN→PT: match against the exact pt field (data is now clean — no slash variants).
      correct = heard === normaliseAnswer(item.pt);
    }
  ...
  if (!quizMode) {
    return (
      <div style={listPanelStyle}>
        ...
        {NUMBERS.map((sec, si) => (
          <div key={si} style={secWrap}>
            <p style={{ ...secTit, color: "var(--color-accent-purple)", fontWeight: 700, textDecoration: "underline" }}>{sec.section}</p>
            <table style={tbl}><tbody>
              {sec.items.map((it, ii) => (
                <tr key={ii} style={{ borderBottom: "1px solid #d1d5db" }}>
                  <td style={{ width: 32, paddingRight: 6, verticalAlign: "middle" }}>
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", padding: "12px 16px", gap: 10 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        {cardBtn("📋 Browse", toggleQuizMode, { background: "#eff6ff", color: "var(--color-accent-blue)", borderColor: "var(--color-border-info)" })}
        {cardBtn("🔀 Shuffle", shuffle)}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", borderRadius: 6, overflow: "hidden", border: "1px solid var(--color-border-tertiary)" }}>
          {[["all","All"],["cardinals","Cardinals"],["other","Other"]].map(([id, label]) => (
            <button key={id} onClick={() => { setCategory(id); resetQuiz(); }} style={segBtnStyle(category === id, "var(--color-accent-purple)")}>{label}</button>
          ))}
        </div>
        {/* Direction */}
        <div style={{ display: "flex", borderRadius: 6, overflow: "hidden", border: "1px solid var(--color-border-tertiary)" }}>
        ...
        <div style={{ textAlign: "center" }}>
          {direction === "pt-en" ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
              ...
              <button onClick={() => speakListPT(item.pt)}
                style={{ fontSize, padding: "10px 28px", borderRadius: 8, border: "1.5px solid #bfdbfe", background: "#eff6ff", color: "var(--color-accent-blue)", cursor: "pointer", fontWeight: 700 }}>
                ▶ Play Portuguese
              </button>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                ...
                <button onClick={() => speakListPT(item.en, "en-US")}
                  style={{ fontSize: Math.max(12, fontSize - 2), padding: "4px 14px", borderRadius: 6, border: "1px solid #d1d5db", background: "var(--color-surface)", cursor: "pointer" }}>▶ Hear English</button>
              </div>
            </div>
          )}
        </div>
            ...
            {(direction === "en-pt" || (direction === "pt-en" && inputMethod === "mic")) && speechSupported && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                <button onClick={micActive ? stopMic : () => startMic(micLang)}
                  style={{ fontSize, padding: "10px 28px", borderRadius: 8, border: micActive ? "2px solid #dc2626" : "1.5px solid var(--color-border-tertiary)", background: micActive ? "var(--color-background-danger)" : "var(--color-background-secondary)", color: micActive ? "var(--color-text-danger)" : "var(--color-text-secondary)", cursor: "pointer", fontWeight: 700 }}>
                ...
                <td style={{ paddingTop: 8, paddingBottom: 4, verticalAlign: "top" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 5 }}>
                    <button onClick={() => speakListPT(item.example_pt)}
                      ...
                      style={{ fontSize: 11, padding: "1px 5px", borderRadius: 4, border: "1px solid #bfdbfe", background: "#eff6ff", color: "var(--color-accent-blue)", cursor: "pointer", lineHeight: 1, flexShrink: 0, marginTop: 2 }}>▶</button>
                    <div>
    ...
    <div style={{ colorScheme: theme === "system" ? "light dark" : theme, fontFamily: "var(--font-sans)", display: "flex", flexDirection: "column", height: "100vh", minHeight: 500, background: "var(--color-background-primary)" }}>
      ...
      {activePanel === "lists" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                                ...
                                <td style={{ width: 32, paddingRight: 4, verticalAlign: "middle" }}>
                                  <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                                    <button onClick={() => speakListPT(item.example, "pt-PT")} title={`PT: ${item.example}`}
                                      style={{ fontSize: 11, padding: "1px 4px", borderRadius: 3, border: "1px solid var(--color-border-tertiary)", background: "var(--color-background-info)", color: "var(--color-accent-blue)", cursor: "pointer", lineHeight: 1 }}>▶PT</button>
                                    <button onClick={() => speakListPT(item.example_en, "en-US")} title={`EN: ${item.example_en}`}
                                      style={{ fontSize: 11, padding: "1px 4px", borderRadius: 3, border: "1px solid var(--color-border-tertiary)", background: "var(--color-background-green)", color: "var(--color-accent-green)", cursor: "pointer", lineHeight: 1 }}>▶EN</button>
                                  </div>
                                </td>
          ...
          {listTab === "numbers" && (
            <NumbersTab fontSize={fontSize} speakListPT={speakListPT} speechSupported={speechSupported} />
          )}
          {listTab === "cognates" && (
