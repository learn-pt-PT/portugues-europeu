...
const APP_META = {
  version: "3.0.34", // ALWAYS update the <!-- version: X.Y.Z --> comment in <head> to match
...
// PRONUNCIATION_GUIDE: displayed in alphabetical section order via PRONUNCIATION_GUIDE_SORTED (see below).
// The source array is kept in the same alphabetical order to match; do not
// insert new sections at an arbitrary position — maintain alphabetical order (key: section).
...
// IPA_GUIDE: displayed in alphabetical section order via IPA_GUIDE_SORTED (see below).
// The source array is kept in the same alphabetical order to match; do not
// insert new sections at an arbitrary position — maintain alphabetical order (key: section).
const IPA_GUIDE = [
  { section: "Consonants", items: [
...
// PHRASES: displayed in alphabetical section order via PHRASES_SORTED (see below).
// The source array is kept in the same alphabetical order to match; do not
// insert new sections at an arbitrary position — maintain alphabetical order.
const PHRASES = [
...
// MEDIA_SECTIONS: displayed in alphabetical order via MEDIA_SECTIONS_SORTED (see below).
// The source array is kept in the same alphabetical order to match; do not
// insert new sections at an arbitrary position — maintain alphabetical order (key: en).
...
// Source: "Ciência e Saúde com a Chave das Escrituras" — Brazilian Portuguese (BP)
...
// SLANG: displayed in alphabetical order via SLANG_SORTED (see below).
// The source array is kept in the same alphabetical order to match; do not
// insert new sections at an arbitrary position — maintain alphabetical order (key: en).
const SLANG = [
...
// Maps GRAMMAR_SECTIONS title strings → GRAMMAR_TOPICS id strings.
// Used by the Grammar tab to nest GRAMMAR_SECTIONS entries under their parent topic
// accordion (line 4394: GRAMMAR_SECTIONS.filter(sec => GRAMMAR_SECTIONS_PARENT_MAP[sec.title] === topic.id)).
// IMPORTANT: if you add a new entry to GRAMMAR_SECTIONS, add its title here too;
// otherwise it will never appear in the UI under any Grammar Focus topic.
...
const GRAMMAR_SECTIONS = [
  ...
  },
  {
    ...
    intro: "Spoken EP rarely uses the bare future tense (falarei). Two alternatives dominate everyday speech: ir + infinitive for near/intended future, and the present tense with a time marker for informal future. The bare future is reserved for formal writing and emphasis.",
    rules: [
      {
        label: "ir + infinitive — the default spoken future",
        explanation: "Conjugate ir (vou, vais, vai, vamos, vão, vão) + a + infinitive. This covers most future contexts in spoken EP.",
        examples: [
          { pt: "Vou falar contigo amanhã.", en: "I’m going to talk to you tomorrow.", note: "" },
          { pt: "O que é que vais fazer ao fim de semana?", en: "What are you going to do at the weekend?", note: "" },
        ],
      },
      {
        label: "Present tense + time marker — informal near future",
        explanation: "With a clear time marker (amanhã, depois, logo, na próxima semana), the present tense expresses the future naturally.",
        examples: [
          { pt: "Falo contigo amanhã.", en: "I’ll talk to you tomorrow.", note: "" },
          { pt: "Chegamos sábado.", en: "We’re arriving Saturday.", note: "" },
        ],
      },
      {
        label: "Futuro do Indicativo — formal and written only",
        explanation: "The synthetic future (falarei, falarás, falará…) sounds stiff in conversation. Use it for formal writing, news, or deliberate emphasis.",
        examples: [
          { pt: "O primeiro-ministro falará amanhã.", en: "The Prime Minister will speak tomorrow.", note: "Typical in news/formal register." },
        ],
      },
    ],
  },
  {
    ...
    intro: "EP uses the Pretérito Perfeito Simples (comi) where English uses the present perfect (‘I’ve eaten’). The compound tenho + particípio exists but means something specific: repeated or ongoing action up to the present. Getting this right is one of the clearest markers of authentic EP.",
    rules: [
      {
        label: "Perfeito Simples — single completed act (use where English uses present perfect)",
        explanation: "EP uses the simple past for any single completed action, including ones English would call present perfect.",
        examples: [
          { pt: "Comi a sopa.", en: "I ate the soup. / I’ve eaten the soup.", note: "Both English forms map to Perfeito Simples in EP." },
          { pt: "Já vi esse filme.", en: "I’ve already seen that film.", note: "" },
        ],
      },
      {
        label: "Imperfeito — habitual or background past",
        explanation: "Use the Imperfeito for repeated past habits, background descriptions, or actions in progress when another event occurred.",
        examples: [
          { pt: "Comia sopa todos os dias.", en: "I used to eat soup every day.", note: "" },
          { pt: "Chovia quando saí.", en: "It was raining when I left.", note: "" },
        ],
      },
      {
        label: "tinha + particípio — the spoken pluperfect",
        explanation: "In speech, the compound form (tinha comido) replaces the literary pluperfect (comera) for ‘had done’.",
        examples: [
          { pt: "Já tinha comido quando chegaste.", en: "I had already eaten when you arrived.", note: "" },
        ],
      },
      {
        label: "tenho + particípio — repeated action continuing to now (not a simple present perfect)",
        explanation: "Unlike English, this form only covers actions that repeat or continue up to the present moment. Never use it for single completed events.",
        examples: [
          { pt: "Tenho comido melhor ultimamente.", en: "I’ve been eating better lately. (repeated recent habit)", note: "" },
          { pt: "Tenho trabalhado muito.", en: "I’ve been working a lot (repeatedly).", note: "" },
        ],
      },
    ],
  },
  {
    ...
    intro: "In EP, using the Imperfeito instead of the present tense significantly softens a request. ‘Quero um café’ is blunt; ‘Queria um café’ is polite and completely natural. This is one of the most useful pragmatic patterns for daily life in Portugal.",
    rules: [
      {
        label: "Softened request — very common at counters and shops",
        explanation: "Use the Imperfeito of querer, pedir, or poder to make any request sound polite rather than demanding.",
        examples: [
          { pt: "Queria um café, faz favor.", en: "I’d like a coffee, please.", note: "Much more natural than ‘Quero um café’ in a shop." },
          { pt: "Podia trazer a conta?", en: "Could you bring the bill?", note: "" },
        ],
      },
      {
        label: "Softened question",
        explanation: "",
        examples: [
          { pt: "Podia repetir, faz favor?", en: "Could you repeat that, please?", note: "" },
          { pt: "Sabia onde fica a estação?", en: "Would you happen to know where the station is?", note: "" },
        ],
      },
      {
        label: "Tentative statement",
        explanation: "The Imperfeito can hedge a statement, making it sound less assertive.",
        examples: [
          { pt: "Pensava que era por aqui.", en: "I was thinking it was this way.", note: "" },
        ],
      },
    ],
  },
...
// Keys must match the 11 tense names requested by buildConjugationPrompt().
// If you add or rename a tense in buildConjugationPrompt, update TENSE_USAGE to match.
const TENSE_USAGE = {
...
function buildConjugationPrompt(verb) {
  // Sanitise verb before interpolation (S-03). Strips any character outside the legal
  // Portuguese verb character set. validateVerbInput already enforces this for error display;
  // this is defence-in-depth at the prompt boundary.
  const safeVerb = verb.replace(/[^a-z\u00e0-\u00ff\- ]/gi, "").trim();
  return `You are a European Portuguese grammar reference. Provide conjugation tables for the verb "${safeVerb}".
...
For each tense, include one natural example sentence using a randomly chosen pronoun. The example must use the correct conjugated form of "${safeVerb}" for that tense. Use European Portuguese naturally — contractions, clitic placement, and vocabulary appropriate for Portugal (not Brazil).
...
// lsSet error callback: populated by App component on mount so the storage-full
// warning is rendered as a React element rather than raw DOM manipulation (C-12).
// Falls back to console.warn only if the callback has not yet been registered.
let _lsSetErrorCallback = null;
function registerLsSetErrorCallback(fn) { _lsSetErrorCallback = fn; }
...
// Validate a raw pe_conj_cache object loaded from localStorage.
// For each cached verb, checks that exactly the 11 expected tense names are present and that
// each tense has the correct row count. Entries failing any check are silently evicted and
// will be re-fetched from the API on next use, preventing stale/corrupt data (CACHE-01).
// Expected row counts: standard tenses = 6 rows; Imperativo = 4 rows; Infinitivo Pessoal = 6 rows.
function validateConjCache(raw) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return {};
  const EXPECTED_TENSES = {
    "Presente do Indicativo":          6,
    "Imperativo":                      4,
    "Pretérito Perfeito Simples":      6,
    "Pretérito Imperfeito do Indicativo": 6,
    "Futuro do Indicativo":            6,
    "Condicional":                     6,
    "Presente do Conjuntivo":          6,
    "Infinitivo Pessoal":              6,
    "Pretérito Mais-que-Perfeito":     6,
    "Pretérito Imperfeito do Conjuntivo": 6,
    "Futuro do Conjuntivo":            6,
  };
  const expectedNames = Object.keys(EXPECTED_TENSES);
  const validated = {};
  for (const [verb, entry] of Object.entries(raw)) {
    if (!entry || typeof entry !== "object") {
      console.warn(`[pe] conjCache evicted "${verb}": entry is not an object`);
      continue;
    }
    const tenses = entry.tenses;
    if (!Array.isArray(tenses) || tenses.length !== 11) {
      console.warn(`[pe] conjCache evicted "${verb}": expected 11 tenses, got ${Array.isArray(tenses) ? tenses.length : typeof tenses}`);
      continue;
    }
    const tenseNames = tenses.map(t => t && t.name);
    const missingTense = expectedNames.find(name => !tenseNames.includes(name));
    if (missingTense) {
      console.warn(`[pe] conjCache evicted "${verb}": missing tense "${missingTense}"`);
      continue;
    }
    let rowFail = null;
    for (const t of tenses) {
      const expectedRows = EXPECTED_TENSES[t.name];
      if (!Array.isArray(t.rows) || t.rows.length !== expectedRows) {
        rowFail = `tense "${t.name}" has ${Array.isArray(t.rows) ? t.rows.length : typeof t.rows} rows, expected ${expectedRows}`;
        break;
      }
    }
    if (rowFail) {
      console.warn(`[pe] conjCache evicted "${verb}": ${rowFail}`);
      continue;
    }
    validated[verb] = entry;
  }
  return validated;
}
...
(function migrateStorage() {
  try {
    ...
    // Swallow all storage errors at startup — lsSet banner will surface them on first write.
    // QuotaExceededError: storage is full. SecurityError / DOMException: storage is blocked
    // (e.g. private-browsing mode with strict settings). Both are non-fatal here.
    if (e && e.name && e.name !== "QuotaExceededError" && e.name !== "SecurityError") {
      console.warn("[pe] migrateStorage unexpected error:", e);
    }
  }
})();
    ...
    <div style={{ padding: "12px 16px" }}>
      ...
      {session && (
        <p style={{ fontSize: 13, color: "var(--color-text-secondary)", marginBottom: 10 }}>
          ...
          <button title={checked ? "Hear correct sentence" : "Available after checking"} aria-label={checked ? "Hear correct sentence" : "Available after checking"} style={{ marginLeft: 8, background: "none", border: "none", fontSize: 15, padding: "0 2px", cursor: checked ? "pointer" : "not-allowed", opacity: checked ? 1 : 0.3 }}
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled || !resp.ok) return;  // !resp.ok: prefetch failure — silent by design (non-fatal background fetch)
        ...
        scheduleConjCacheWrite();
      } catch { /* silent — prefetch failures are non-fatal; background fetch only */ }
    })();
        ...
        {missedItems.length > 0 && (
          <div style={_vqCardStyle()}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize }}>
              <tbody>
                {missedItems.map((r) => (
                  <tr key={r.item.tenseName + "-" + r.item.pronoun} style={{ borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
...
// shuffleArr removed in 3.0.16 — use shuffleArray
...
const OppositesQuiz = React.memo(function OppositesQuiz({ fontSize, speakListPT, speechSupported, onBack }) {
  ...
  React.useEffect(() => {
    ...
    const all = shuffleArray([item.answer.pt, ...distractors]);
    setChoices(all);
    setResult(null);
    setHeardText("");
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "12px 16px", gap: 10, overflowY: "auto" }}>
      ...
      <div style={{ background: "var(--color-background-secondary)", border: "1px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-md)", padding: "18px 20px", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          <button onClick={() => speakListPT(item.prompt.pt)} aria-label={"Ouvir: " + item.prompt.pt} style={{ background: "transparent", border: "none", cursor: "pointer", fontSize: fontSize + 4, color: "var(--color-accent-blue)" }}>🔊</button>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {showSentence && (
            <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 10px", background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", border: "0.5px solid var(--color-border-tertiary)" }}>
              <button onClick={() => speakListPT(item.sentence)} aria-label="Ouvir frase de exemplo" style={{ background: "transparent", border: "none", cursor: "pointer", fontSize, color: "var(--color-accent-blue)", flexShrink: 0 }}>🔊</button>
...
const MinimalPairs = React.memo(function MinimalPairs({
  ...
  // Intentionally simpler than main chat SR (see createRecognition, v3.0.1–3.0.5 fix history).
  // Single-shot: stops on first final result. Does not use sendingRef/finalTranscriptRef/
  // restart-counter guard. Do not add continuous-mode logic here without reviewing main chat SR first.
  function startMicGuess() {
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
...
const CultureNotesAccordion = React.memo(function CultureNotesAccordion({ fontSize, listFilter }) {
  ...
  const visibleNotes = CULTURE_NOTES.reduce((acc, cn, si) => {
    if (!needle) { acc.push({ cn, si, matchedItems: cn.items.map((item, ii) => ({ item, ii })) }); return acc; }
    const sectionMatch = cn.section.toLowerCase().includes(needle);
    const matchedItems = cn.items.map((item, ii) => ({ item, ii })).filter(({ item }) =>
      item.title.toLowerCase().includes(needle) || item.body.toLowerCase().includes(needle)
    );
    if (sectionMatch || matchedItems.length > 0) {
      acc.push({ cn, si, matchedItems: sectionMatch ? cn.items.map((item, ii) => ({ item, ii })) : matchedItems });
    }
    return acc;
  }, []);
  ...
  return (
    <div>
      {visibleNotes.map(({ cn, si, matchedItems }) => {
        const isSecOpen = needle ? true : openSection === si;
        return (
          <div key={si} style={{ marginBottom: 8, border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-md)", overflow: "hidden" }}>
            <button onClick={() => toggleSection(si)} aria-expanded={isSecOpen}
              ...
              <span style={{ fontSize: 12, color: "var(--color-text-tertiary)", flexShrink: 0 }}>{isSecOpen ? "▲" : "▼"}</span>
            </button>
            {isSecOpen && (
              <div style={{ padding: "8px 12px 12px" }}>
                {matchedItems.map(({ item, ii }) => {
                  const isItemOpen = needle ? true : !!openItem[`${si}-${ii}`];
                  return (
                    <div key={ii} style={{ marginBottom: 6, border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-sm)", overflow: "hidden" }}>
                      <button onClick={() => toggleItem(si, ii)} aria-expanded={isItemOpen}
                        ...
                        <span style={{ fontSize: 11, color: "var(--color-text-tertiary)", flexShrink: 0 }}>{isItemOpen ? "▲" : "▼"}</span>
                      </button>
                      {isItemOpen && (
...
const GrammarListTab = React.memo(function GrammarListTab({ fontSize, speakListPT, onPractice }) {
      ...
      {GRAMMAR_TOPICS.map((topic) => {
        ...
        return (
          <div key={topic.id} style={{ marginBottom: 6, border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-md)", overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", background: "var(--color-background-secondary)" }}>
              <button onClick={() => toggleTopic(topic.id)} aria-expanded={isTopicOpen} style={{
              ...
              {onPractice && (
                <button
                  onClick={(e) => { e.stopPropagation(); onPractice(topic.label); }}
                  style={{ fontSize: 11, fontWeight: 600, padding: "3px 8px", marginRight: 8, borderRadius: "var(--border-radius-md)", border: "1px solid var(--color-accent-blue)", background: "transparent", color: "var(--color-accent-blue)", cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
                  Practice in chat
                </button>
              )}
            </div>
                    ...
                    {nestedSections.map((sec, nsi) => {
                      ...
                      return (
                        <div key={nsi} style={{ marginBottom: 6, border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-sm)", overflow: "hidden" }}>
                          <button onClick={() => toggleNestedSec(sec.title)} aria-expanded={isSecOpen} style={{
                          ...
                          {isSecOpen && (
                            <div style={{ padding: "8px 12px 12px", background: "var(--color-background-primary)" }}>
                              ...
                              {sec.rules.map((rule, ri) => {
                                ...
                                return (
                                  <div key={ri} style={{ marginBottom: 6, border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-sm)", overflow: "hidden" }}>
                                    <button onClick={() => toggleNestedRule(sec.title, ri)} aria-expanded={isRuleOpen} style={{
                                      width: "100%", textAlign: "left", background: "var(--color-background-secondary)",
                                      border: "none", cursor: "pointer", padding: "6px 10px",
                                      display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8,
                                    }}>
...
const MediaTab = React.memo(function MediaTab({ fontSize, mediaOpenSection, setMediaOpenSection, mediaSectionRefs, listFilter }) {
  const { useMemo } = React;
  ...
  const needle = (listFilter || "").toLowerCase();
  const visibleSections = MEDIA_SECTIONS_SORTED.reduce((acc, sec) => {
    if (!needle) { acc.push({ sec, sortedLinks: sec.links ? [...sec.links].sort((a, b) => a.label.localeCompare(b.label)) : null, sortedSubgroups: sec.subgroups ? sec.subgroups.map(sg => ({ ...sg, links: [...sg.links].sort((a, b) => a.label.localeCompare(b.label)) })) : null }); return acc; }
    const filteredLinks = sec.links ? sec.links.filter(lk => lk.label.toLowerCase().includes(needle)).sort((a, b) => a.label.localeCompare(b.label)) : null;
    const filteredSubgroups = sec.subgroups ? sec.subgroups.map(sg => ({ ...sg, links: sg.links.filter(lk => lk.label.toLowerCase().includes(needle)).sort((a, b) => a.label.localeCompare(b.label)) })).filter(sg => sg.links.length > 0) : null;
    const headerMatch = sec.en.toLowerCase().includes(needle) || (sec.pt && sec.pt.toLowerCase().includes(needle));
    if (headerMatch) {
      acc.push({ sec, sortedLinks: sec.links ? [...sec.links].sort((a, b) => a.label.localeCompare(b.label)) : null, sortedSubgroups: sec.subgroups ? sec.subgroups.map(sg => ({ ...sg, links: [...sg.links].sort((a, b) => a.label.localeCompare(b.label)) })) : null });
    } else if ((filteredLinks && filteredLinks.length > 0) || (filteredSubgroups && filteredSubgroups.length > 0)) {
      acc.push({ sec, sortedLinks: filteredLinks, sortedSubgroups: filteredSubgroups });
    }
    return acc;
  }, []);
  ...
  return (
    <div style={listPanelStyle}>
      {visibleSections.map(({ sec, sortedLinks, sortedSubgroups }) => {
        const isOpen = needle ? true : mediaOpenSection === sec.id;
        return (
          <div key={sec.id} ref={el => mediaSectionRefs.current[sec.id] = el} style={{ marginBottom: 10, border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-md)", overflow: "hidden" }}>
            <button onClick={() => {
              ...
              <span style={{ fontSize: 12, color: "var(--color-text-tertiary)", flexShrink: 0 }}>{isOpen ? "▲" : "▼"}</span>
            </button>
            {isOpen && (
              <div style={{ padding: "8px 12px 12px" }}>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        <CultureNotesAccordion fontSize={fontSize} listFilter={listFilter} />
      </div>
    </div>
  );
});
      ...
      {filteredSections.map((sec, si) => {
        ...
        return (
          <div key={si} style={{ marginBottom: 5, border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-md)", overflow: "hidden" }}>
            <button onClick={() => togglePhrSec(sec.section)} aria-expanded={isOpen} style={{
              width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center",
    ...
    <div style={listPanelStyle}>
      ...
      <table style={tbl}><tbody>
        {visible.map((item) => {
          ...
          return (
            <tr key={item.pt} style={{ borderBottom: "1px solid var(--color-border-tertiary)" }}>
              <td style={{ width: 32, paddingRight: 6, verticalAlign: "middle" }}>
                <button onClick={() => speakListPT(item.pt)}
                  aria-label={`Hear: ${item.pt}`}
                  style={{ fontSize: 13, padding: "1px 5px", borderRadius: 4, border: "1px solid var(--color-border-tertiary)", background: "var(--color-surface)", cursor: "pointer", lineHeight: 1 }}>▶</button>
...
function App() {
  ...
  // Register the lsSet error callback so storage-full warnings surface as
  // a React-rendered banner rather than raw DOM manipulation (C-12 resolved).
  useEffect(() => {
    registerLsSetErrorCallback(() => setStorageFull(true));
    return () => registerLsSetErrorCallback(null);
  }, []);
  ...
  const conjCache = useRef(validateConjCache(ls("pe_conj_cache", {})));  // persisted across sessions; validated on load (CACHE-01)
  const [conjCacheCount, setConjCacheCount] = useState(() => Object.keys(validateConjCache(ls("pe_conj_cache", {}))).length);
  // P-03: debounce conjCache writes — coalesces rapid cache updates into a single localStorage write.
  // scheduleConjCacheWrite() replaces all four direct lsSet("pe_conj_cache", ...) call sites.
  // The explicit cache clear in clearConjCache() writes {} directly and bypasses this debounce intentionally.
  const conjCacheWriteTimerRef = useRef(null);
  const scheduleConjCacheWrite = useCallback(() => {
    if (conjCacheWriteTimerRef.current) clearTimeout(conjCacheWriteTimerRef.current);
    conjCacheWriteTimerRef.current = setTimeout(() => {
    ...
    r.onresult = (e) => {
      ...
      for (let i = e.resultIndex; i < e.results.length; i++) {
        if (e.results[i].isFinal) {
          ...
          // Reset restart counter on each successful final result — prevents the 5-restart
          // cap from being reached by accumulated silence gaps across a long dictation session.
          if (restartCountRef) restartCountRef.current = 0;
        } else {
          interim += e.results[i][0].transcript;
        }
      }
  ...
  const addCustomTopic = () => {
    ...
    // Sanitise label before interpolation into system prompt (S-03).
    // Permits letters (incl. accented), digits, spaces, hyphens, apostrophes, commas, periods.
    const safeLabel = trimmed.replace(/[^\w\s\-',.\u00C0-\u00FF]/g, "").trim();
    if (safeLabel && safeLabel.length <= 60 && topics.length < 50 && !topics.find(t => t.label.toLowerCase() === safeLabel.toLowerCase())) {
      setTopics(prev => { const next = [...prev, { label: safeLabel, selected: true }]; lsSet("pe_topics", next); return next; });
      setCustomTopic("");
    }
  };
  ...
  const prefetchVocabCard = async (word, pos, signal) => {
    ...
    try {
      ...
      if (!resp.ok) return; // non-fatal background prefetch — silently discard HTTP errors
      const data = await resp.json();
  ...
  return (
    ...
    <div style={{ colorScheme: theme === "system" ? "light dark" : theme, fontFamily: "var(--font-sans)", display: "flex", flexDirection: "column", flex: 1, minWidth: 0, height: "100vh", minHeight: 500, background: "var(--color-background-primary)" }}>
      ...
      {/* Storage-full banner — rendered as React state (C-12) */}
      {storageFull && (
        <div role="alert" style={{ position: "fixed", bottom: 12, left: "50%", transform: "translateX(-50%)", zIndex: 99999, background: "var(--color-background-warning)", color: "var(--color-text-warning)", border: "1px solid #fcd34d", borderRadius: 8, padding: "10px 18px", fontSize: 13, boxShadow: "0 4px 12px rgba(0,0,0,0.15)", maxWidth: "90vw", textAlign: "center", cursor: "pointer" }}
          title="Click to dismiss" onClick={() => setStorageFull(false)}>
          ⚠ Settings could not be saved — storage is full or unavailable (e.g. private browsing). Changes will be lost on reload.
        </div>
      )}
      ...
      {activePanel === "settings" && (
        <div style={{ ...panelStyle, flex: 1, maxHeight: "none" }}>
            ...
            {apiKey ? (
              ...
              <div style={{ display: "flex", gap: 6 }}>
                ...
                <button style={{ ...toolBtn(true), background: "var(--color-accent-blue)", color: "#fff", padding: "5px 14px" }}
                  onClick={() => { const k = apiKeyInput.trim().replace(/[^A-Za-z0-9\-_]/g, ""); if (k) { if (!k.startsWith("sk-ant-")) { console.warn("[pe] Saved key does not start with sk-ant- — verify this is an Anthropic API key"); } setApiKey(k); lsSet("pe_api_key", k); setApiKeyInput(""); if (location.protocol !== "https:" && location.hostname !== "localhost" && location.hostname !== "127.0.0.1") { console.warn("[pe] API key saved over non-HTTPS origin"); } } }}>Save</button>
              </div>
            )}
            ...
            {azureKey ? (
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                ...
                <button style={{ ...toolBtn(true), background: "var(--color-accent-violet)", color: "#fff", padding: "5px 14px" }}
                  onClick={() => { const k = azureKeyInput.trim().replace(/[^A-Za-z0-9\-_]/g, ""); if (k && azureRegion) { setAzureKey(k); lsSet("pe_azure_key", k); setAzureKeyInput(""); setSelectedVoice({ name: "__azure_raquel__", lang: "pt-PT", azureVoice: "pt-PT-RaquelNeural" }); } }}>Save</button>
              </div>
            )}
            <p style={{ fontSize: Math.max(11, fontSize - 3), color: "var(--color-text-warning)", margin: "5px 0 0", fontStyle: "italic", background: "var(--color-background-warning)", padding: "4px 8px", borderRadius: "var(--border-radius-sm)" }}>
          ...
          <div style={{ marginBottom: 8 }}>
            {topics.map((t, i) => (
              <button key={t.label} style={chip(t.selected)} onClick={() => toggleTopic(i)}>
      ...
      {activePanel === "lists" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
              ...
              {!selectedVerb ? (
                ...
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                    ...
                    <button
                      style={{ marginLeft: "auto", fontSize: 11, fontWeight: 600, padding: "3px 10px", borderRadius: "var(--border-radius-md)", border: "1px solid var(--color-accent-blue)", background: "transparent", color: "var(--color-accent-blue)", cursor: "pointer", whiteSpace: "nowrap" }}
                      onClick={() => {
                        const prompt = `Give me 5 European Portuguese sentences using "${selectedVerb}". List the 5 Portuguese sentences separated by blank lines, with no numbering or bullet points. Then output the separator === on its own line. Then list the English translations in the same order, separated by blank lines, with no numbering or bullet points. Do not add any other text after the ===.`;
                        setActivePanel(null);
          ...
          {listTab === "grammar" && (
            <GrammarListTab fontSize={fontSize} speakListPT={speakListPT} onPractice={(label) => {
                const prompt = `Give me 5 European Portuguese sentences that illustrate ${label}. List the 5 Portuguese sentences separated by blank lines, with no numbering or bullet points. Then output the separator === on its own line. Then list the English translations in the same order, separated by blank lines, with no numbering or bullet points. Do not add any other text after the ===.`;
                setActivePanel(null);
                setTimeout(() => { sendMessageRef.current?.(prompt); }, 100);
              }} />
          )}
      ...
      {activePanel === "verblookup" && (
        <div style={{ ...listPanelStyle, display: "flex", flexDirection: "column", overflow: "hidden", padding: 0 }}>
          ...
          {conjError && <p role="alert" style={{ fontSize: 16, color: "var(--color-text-danger)", margin: "0 16px" }}>{conjError}</p>}
          {conjugation && (
            <div style={{ flex: 1, overflowY: "auto", padding: "0 16px 16px" }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 12, flexWrap: "wrap", margin: "0 0 12px" }}>
                ...
                <button
                  style={{ fontSize: 13, fontWeight: 600, padding: "3px 12px", borderRadius: 999, border: "1px solid var(--color-accent-blue)", background: "transparent", color: "var(--color-accent-blue)", cursor: "pointer", fontFamily: "var(--font-sans)", flexShrink: 0 }}
                  onClick={() => {
                    const prompt = `Give me 5 European Portuguese sentences using "${conjugation.infinitive}". List the 5 Portuguese sentences separated by blank lines, with no numbering or bullet points. Then output the separator === on its own line. Then list the English translations in the same order, separated by blank lines, with no numbering or bullet points. Do not add any other text after the ===.`;
                    setActivePanel(null);
                    setTimeout(() => { sendMessageRef.current?.(prompt); }, 100);
                  }}>
                  Practice in chat
        ...
        <div style={{ ...listPanelStyle, display: "flex", flexDirection: "column", gap: 10 }}>
          {!vocabShowReview ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {vocabError && <p role="alert" style={{ fontSize: 16, color: "var(--color-text-danger)", margin: 0 }}>{vocabError}</p>}
              {vocabCard && (
            ...
            <div>
              ...
              {vocabReview.filter(c => c.is_bp_specific).length > 0 && (
                <div style={{ marginBottom: 14 }}>
                  ...
                  <table style={tbl}><tbody>
                    {vocabReview.filter(c => c.is_bp_specific).map((card) => (
                      <tr key={card.word_bp} style={{ borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
