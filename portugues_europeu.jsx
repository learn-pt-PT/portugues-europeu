...
const IdiomsTab = React.memo(function IdiomsTab({ fontSize, speakListPT, listFilter }) {
  ...
  const tdR = useMemo(() => ({ color: "var(--color-text-secondary)", padding: "3px 0", verticalAlign: "top", fontSize }), [fontSize]);
  const needle = (listFilter || "").toLowerCase();
  const visible = useMemo(() => needle ? IDIOMS.filter(it => it.pt.toLowerCase().includes(needle) || it.en.toLowerCase().includes(needle) || (it.when && it.when.toLowerCase().includes(needle))) : IDIOMS, [needle]);
  return (
    <div style={listPanelStyle}>
      ...
      {visible.length === 0 && needle && <p style={{ fontSize, color: "var(--color-text-tertiary)", fontStyle: "italic" }}>No matches.</p>}
      <table style={tbl}><tbody>
        {visible.map((item, i) => (
