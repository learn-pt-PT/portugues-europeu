...
function renderWithParens(text, baseColor, baseWeight, fontSize) {
  ...
  return parts.map((part, pi) => {
    if (/^\([^)]+\)$/.test(part)) {
      ...
      return isCorrection
        ? <span key={pi} style={{ color: "#800000", fontWeight: baseWeight, fontStyle: "normal" }}>{inner}</span>
        : <span key={pi} style={{ color: "#166534", fontWeight: baseWeight, fontStyle: "normal" }}>{inner}</span>;
    }
...
const MessageBubble = React.memo(function MessageBubble({ m, fontSize, ttsSupported, speak, showTranslation }) {
  ...
  const correctionEl = endCorrection ? (
    <span style={{ color: "#800000", fontWeight: 700, fontStyle: "normal" }}>
              ...
              {vocabCard && (
                <div style={{ border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-md)", overflow: "hidden" }}>
                  ...
                  <div style={{ padding: "8px 14px", borderTop: "0.5px solid var(--color-border-tertiary)", display: "flex", gap: 6 }}>
                    ...
                    <button style={{ ...toolBtn(false), color: inReview ? "#92400e" : "var(--color-text-secondary)" }} onClick={toggleReview}>
